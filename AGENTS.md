...
Add instructions that apply to all Capy agents.
Architect MCP Logo
...
Create: Your AI agent needs to accomplish something new, so it writes the JavaScript code to do it.
Review & Approve: You review the tool's requested permissions and click "Approve", granting it secure network or file access. No rogue scripts allowed.
Execute: The agent runs its shiny new tool safely inside the isolated sandbox.
Automate & Share: The agent can set the tool to run on a schedule, chain it with other tools, or publish it to the global marketplace.